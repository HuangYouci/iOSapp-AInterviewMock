...
class DataManager: ObservableObject {
    ...
    /// Update JSON
    func saveInterviewProfileJSON(_ target: InterviewProfile) {
        let encoder = JSONEncoder()
        if let data = try? encoder.encode(target) {
            let url = FileManager.default
                .urls(for: .documentDirectory, in: .userDomainMask)[0]
                .appendingPathComponent("InterviewProfileDocuments")
                .appendingPathComponent("\(target.id.uuidString)")
    ...
    /// **Speech**
    /// Save Document
    func saveSpeechProfileDocuments(speechProfile: inout SpeechProfile) {
            let fileManager = FileManager.default
            ...
            // ~/documents/SpeechProfileDocuments/<interviewProfile.id>/
            let folderURL = documentsURL
                .appendingPathComponent("SpeechProfileDocuments")
                .appendingPathComponent("\(speechProfile.id.uuidString)") // Use interviewProfile
            ...
            // 遍歷 filesPath 使用索引來更新
            for index in speechProfile.filesPath.indices {
                let originalPathString = speechProfile.filesPath[index]
                ...
                // 產生目標檔案名稱與路徑，嘗試保留原始副檔名
                let fileExtension = sourceURL.pathExtension
                let actualExtension = fileExtension.isEmpty ? "pdf" : fileExtension
                let destinationFilename = "\(UUID().uuidString).\(actualExtension)"
                let destinationURL = folderURL.appendingPathComponent(destinationFilename)
                ...
                do {
                    ...
                    // 4. 更新 InterviewProfile 實例中的路徑
                    speechProfile.filesPath[index] = destinationURL.path
                    print("DataManager | filesPath 索引 \(index) 已更新為: \(destinationURL.path)")
    ...
    /// Save Audio (from Temporary)
    func saveSpeechProfileAudios(speechProfile: inout SpeechProfile) {
        ...
        // 2. 建立目標資料夾路徑: ~/documents/SpeechProfileDocuments/<interviewProfile.id>/
        let folderURL = documentsURL
            .appendingPathComponent("SpeechProfileDocuments")
            .appendingPathComponent("\(speechProfile.id.uuidString)")
        ...
        // 4. 把 contentAudioPath 轉存
        do {
            let sourceAudioPathString = speechProfile.speechAudioPath
            if sourceAudioPathString.isEmpty {
                print("DataManager | sourceAudioPath is Empty, skipping.")
            }
            let sourceAudioURL = URL(fileURLWithPath: sourceAudioPathString)
            ...
            do {
                ...
                try fileManager.copyItem(at: sourceAudioURL, to: destinationURL)
                print("DataManager | 音訊複製成功！儲存位置：\(destinationURL.path)")
                speechProfile.speechAudioPath = destinationURL.path
                print("DataManager | speechAudioPath 已更新為: \(destinationURL.path)")
            } catch {
                print("DataManager | 複製音訊檔案失敗：\(error) (從 \(sourceAudioURL.path) 到 \(destinationURL.path))")
            }
        }

        // 5. 遍歷 asked questions，使用索引來修改
        for index in speechProfile.askedQuestions.indices {
            let sourceAudioPathString = speechProfile.askedQuestions[index].answerAudioPath
 
            if sourceAudioPathString.isEmpty {
                print("DataManager | 問題 (ID: \(speechProfile.askedQuestions[index].id)) 的 answerAudioPath 為空，跳過。")
                continue
            }
            ...
            do {
                ...
                speechProfile.askedQuestions[index].answerAudioPath = destinationURL.path
                print("DataManager | 問題 (ID: \(speechProfile.askedQuestions[index].id)) 的 answerAudioPath 已更新為: \(destinationURL.path)")
        ...
        print("DataManager | 所有音訊處理完畢。")
    }
    /// Update JSON
    func saveSpeechProfileJSON(_ target: SpeechProfile) {
        let encoder = JSONEncoder()
        if let data = try? encoder.encode(target) {
            let url = FileManager.default
                .urls(for: .documentDirectory, in: .userDomainMask)[0]
    ...
    func loadAllSpeechProfiles() -> [SpeechProfile] {
        let fileManager = FileManager.default
        var speechs: [SpeechProfile] = []
        let decoder = JSONDecoder()

        guard let baseFolderURL = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first?
            .appendingPathComponent("SpeechProfileDocuments") else {
            print("DataManager | 無法取得 SpeechProfileDocuments 目錄路徑。")
            return []
        }

        guard fileManager.fileExists(atPath: baseFolderURL.path) else {
            print("DataManager | SpeechProfileDocuments 目錄不存在於：\(baseFolderURL.path)")
            return []
        }
            ...
            for potentialFolderURL in subDirectoryURLs {
                ...
                if fileManager.fileExists(atPath: potentialFolderURL.path, isDirectory: &isDirectory), isDirectory.boolValue {
                    ...
                    if fileManager.fileExists(atPath: profileJsonURL.path) {
                        do {
                            let jsonData = try Data(contentsOf: profileJsonURL)
                            let speech = try decoder.decode(SpeechProfile.self, from: jsonData)
                        ...
                        // print("DataManager | 在目錄 \(potentialFolderURL.path) 中未找到 profile.json。")
                    }
                }
            }
        } catch {
            print("DataManager | 獲取 SpeechProfileDocuments 目錄內容失敗: \(error)")
        }

        print("DataManager | 共載入 \(speechs.count) 筆 SpeechProfile。")
        return speechs.sorted(by: { $0.date > $1.date }) // 按日期降序排序，最新的在前
    }
    ...
    func deleteSpeechProfile(withId id: String) {
        let fileManager = FileManager.default
        ...
        // 2. 構造目標資料夾的路徑: ~/documents/SpeechProfileDocuments/<id.uuidString>/
        let folderURLToDelete = documentsURL
            .appendingPathComponent("SpeechProfileDocuments")
            .appendingPathComponent(id)
